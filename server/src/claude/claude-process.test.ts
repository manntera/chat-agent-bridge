...
function createTestContext() {
  ...
  const claudeProcess = new ClaudeProcess(
    { claudePath: '/usr/bin/claude', systemPromptAppend: 'Discord互換の構文のみ使用してください' },
...
describe('ClaudeProcess', () => {
  ...
  describe('spawn', () => {
    ...
    it('--append-system-prompt で設定されたプラットフォーム指示が含まれる', () => {
      const ctx = createTestContext();
    ...
    it('systemPromptAppend が空文字なら --append-system-prompt を付与しない', () => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const spawnFn = vi.fn((...args: [string, string[], SpawnOptions]) => {
        return new MockChildProcess() as unknown as ChildProcess;
      });
      const proc = new ClaudeProcess(
        { claudePath: '/usr/bin/claude', systemPromptAppend: '' },
        () => {},
        () => {},
        spawnFn,
      );

      proc.spawn('hello', 'session-123', '/work', false);
