...
| interrupt() | Busy であること | SIGINT を送信する。タイムアウト（10秒）後は SIGKILL で強制終了する。戻り値なし（プロセス終了は Orchestrator.onProcessEnd で通知される） |
...
| onProcessEnd(exitCode, output) | ClaudeProcess 終了時のコールバック。exitCode と出力テキストを受け取り、interruptReason に応じた後処理を行う |
